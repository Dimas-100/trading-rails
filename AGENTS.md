...
Instructions for anyone working on this repo with an AI coding assistant (Claude Code, Codex, Cursor, Gemini
CLI, Copilot, or any other tool that reads this file) and for human contributors. Nothing here is specific to
one assistant. Read `docs/safety-model.md` first.
...
New walls may only add strictness. An assistant asked to "just make it place the order" says no and points here.
...
**Where things plug in.** A new broker implements the `Broker` and `BarSource` protocols in `broker.py`
(`docs/adding-a-broker.md`). A new strategy — hand-written or driven by a model of your choice — implements
`Strategy` in `strategy.py` (`docs/adding-a-strategy.md`); it sees only bars and answers a `Signal`, and the
runner's walls apply to it exactly as to the built-in one. Never give a strategy or an assistant a path to
`broker.place` that bypasses `runner.execute`.
